...
ARCHITECTURE ripple OF BIST IS
	...
	SIGNAL correctgreater, correctless, correctequal, myResult1_equal, myresult2_greater, myresult3_less : STD_LOGIC := '0';
BEGIN
	-- 56*2 = 112 ns is the minimum delay we should have to have a correct output
	CLK <= NOT CLK AFTER 60ns;--clock speed =120ns < delay(112ns)
	G1 : ENTITY WORK.TestGenerator(generator) PORT MAP(CLK, A, B, correctless, correctgreater, correctequal);
	G2 : ENTITY WORK. comparator_from_RA(comparator_from_RA_arch) PORT MAP(A, B, clk, myResult1_equal, myresult2_greater, myresult3_less);
	G3 : ENTITY WORK.ResultAnalyser(analyser) PORT MAP(CLK, correctgreater, correctless, myResult1_equal, myresult2_greater, myresult3_less);
END;
-----------------------------------------------
